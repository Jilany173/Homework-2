...
// Extract all {id, number} from any group type (including notes_completion & table_completion)
const getGroupQuestions = (g: any): { id: string; number: number }[] => {
    if (g.type === 'notes_completion') {
        return (g.notes || []).filter((n: any) => n.inline).map((n: any) => ({ id: n.inline.inputId, number: n.inline.number }));
    }
    if (g.type === 'table_completion') {
        const qs: { id: string; number: number }[] = [];
        (g.rows || []).forEach((row: any) => (row.cells || []).forEach((cell: any) => (cell.bullets || []).forEach((b: any) => { if (b.inline) qs.push({ id: b.inline.inputId, number: b.inline.number }); })));
        return qs;
    }
    return (g.questions || []).map((q: any) => ({ id: q.id, number: q.number }));
};
...
const ResultsScreen: React.FC<ResultsProps> = ({ answers, answerKey, onBack }) => {
    ...
    // Flatten all questions using universal helper
    const allQs: { qNum: number; studentAns: string; correctAns: string; isCorrect: boolean; isBlank: boolean; passageIndex: number }[] = [];
    testData.passages.forEach((p, pIndex) =>
        p.questionGroups.forEach((g: any) =>
            getGroupQuestions(g).forEach(q => {
                const ans = answers[q.id] || '—';
                const correct = hasKey ? (answerKey[q.number] || '—') : '—';
    ...
    allQs.sort((a, b) => a.qNum - b.qNum);

    const attemptedQs = allQs.filter(q => !q.isBlank);
    const attemptedCount = attemptedQs.length;
    const correctCount = allQs.filter(q => q.isCorrect).length;

    const getBand = (n: number) =>
        n >= 39 ? 9 : n >= 37 ? 8.5 : n >= 35 ? 8 : n >= 33 ? 7.5 : n >= 30 ? 7 :
            n >= 27 ? 6.5 : n >= 23 ? 6 : n >= 20 ? 5.5 : n >= 16 ? 5 : '< 5';

    const band = hasKey ? getBand(correctCount) : 'N/A';
    const activeQ = attemptedQs[activeQuestionIndex];
                        ...
                        <div className="flex flex-col md:flex-row items-center gap-6 z-10 w-full">
                            <div className="text-center md:text-left flex-1">
                                ...
                                <p className="text-sm font-medium text-indigo-600 mt-1 flex items-center justify-center md:justify-start gap-2">
                                    <span className="w-2 h-2 rounded-full bg-indigo-500"></span> Reading Test Results
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="md:col-span-1 space-y-4">
                            ...
                            <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex items-center justify-between">
                                <div>
                                    ...
                                    <div className="text-2xl font-black text-slate-700">{allQs.length}</div>
                                </div>
                        ...
                        <div className="md:col-span-2">
                            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden h-full">
                                <div className="bg-slate-50/80 px-6 py-4 border-b border-slate-100">
                                    <h3 className="font-bold text-slate-700">Passage Breakdown</h3>
                                </div>
                                <div className="p-0 overflow-x-auto">
                                    <table className="w-full text-left text-sm">
                                        <thead className="bg-white text-slate-400 text-xs uppercase tracking-wider font-bold border-b border-slate-100">
                                            <tr>
                                                <th className="px-6 py-4 font-bold">Passage</th>
                                        ...
                                        <tbody className="divide-y divide-slate-50">
                                            {testData.passages.map((passage, pIdx) => {
                                                const pFiltered = allQs.filter(q => q.passageIndex === pIdx);
                                                const pAttempted = pFiltered.filter(q => !q.isBlank).length;
                                                const pCorrect = pFiltered.filter(q => q.isCorrect).length;
                                                const qCount = pFiltered.length;
                                                return (
                                                    <tr key={pIdx} className="hover:bg-slate-50/50 transition-colors">
                                                        <td className="px-6 py-4 font-semibold text-slate-700">Reading Passage {pIdx + 1}</td>
                                                        <td className="px-6 py-4 text-center text-slate-500">{qCount}</td>
                                    ...
                                    <div className="max-w-3xl mx-auto">
                                        <div className="bg-slate-50 rounded-2xl p-8 border border-slate-100 relative overflow-hidden">
                                                ...
                                                {hasKey && (
                                                    <div className="space-y-2">
                                                        ...
                                                        <div className="w-full bg-emerald-50 border-2 border-emerald-100 p-5 rounded-xl text-lg font-bold text-emerald-700 shadow-sm">
                                                            {activeQ.correctAns.split('|').join('  OR  ')}
                                                        </div>
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            <div className="flex justify-between items-center mt-10 pt-6 border-t border-slate-200/60">
                                                ...
                                                <button
                                                    onClick={() => setActiveQuestionIndex(Math.min(attemptedQs.length - 1, activeQuestionIndex + 1))}
                                                    disabled={activeQuestionIndex === attemptedQs.length - 1}
            ...
            if (block.tag === 'heading') {
                return (
                    <div key={i} className="mb-4">
                        <span className="font-black text-slate-800 mr-2">{block.text}</span>
                        <span className="text-slate-700">{block.content}</span>
                    </div>
                );
            }
            return <p key={i} className="mb-3 text-slate-700">{block.text}</p>;
        })}
    </div>
);

// ─── Question Group Renderers ─────────────────────────────────────────────────
const TFNGGroup: React.FC<{ group: any; answers: Record<string, string>; onChange: (id: string, v: string) => void; labels?: string[] }> = ({ group, answers, onChange, labels }) => {
    const opts = labels || ['TRUE', 'FALSE', 'NOT GIVEN'];
    return (
        <div className="mb-8">
            {group.questions.map((q: any) => (
                <div key={q.id} className="mb-5 p-4 bg-white rounded-2xl border border-slate-200 shadow-sm">
                    <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Question {q.number}</p>
                    <p className="text-sm font-medium text-slate-800 mb-3">{q.statement || q.description}</p>
                    <div className="flex flex-wrap gap-2">
                        {opts.map(opt => (
                            <button key={opt} onClick={() => onChange(q.id, opt)}
                                className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider border transition-all ${answers[q.id] === opt ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-slate-50 text-slate-500 border-slate-200 hover:border-blue-300 hover:text-blue-600'}`}>
                                {opt}
                            </button>
                        ))}
                    </div>
...
const MCQGroup: React.FC<{ group: any; answers: Record<string, string>; onChange: (id: string, v: string) => void }> = ({ group, answers, onChange }) => (
    <div className="mb-8">
        {group.questions.map((q: any) => (
            <div key={q.id} className="mb-5 p-4 bg-white rounded-2xl border border-slate-200 shadow-sm">
                <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Question {q.number}</p>
                ...
                <div className="space-y-2">
                    {q.options.map((o: any) => (
                        <button key={o.label} onClick={() => onChange(q.id, o.label)}
                            className={`w-full text-left flex items-start gap-3 px-4 py-3 rounded-xl border text-sm transition-all ${answers[q.id] === o.label ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-slate-50 text-slate-700 border-slate-200 hover:border-blue-300'}`}>
                            <span className="font-black flex-shrink-0">{o.label}</span>
                            <span>{o.text}</span>
                        </button>
                    ))}
                </div>
...
const ShortAnswerGroup: React.FC<{ group: any; answers: Record<string, string>; onChange: (id: string, v: string) => void }> = ({ group, answers, onChange }) => (
    <div className="mb-8">
        {group.questions.map((q: any) => (
            <div key={q.id} className="mb-5 p-4 bg-white rounded-2xl border border-slate-200 shadow-sm">
                <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Question {q.number}</p>
                ...
                <input
                    ...
                    className="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm font-medium text-slate-800 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all"
                />
            </div>
        ))}
    </div>
...
const MatchingHeadingsGroup: React.FC<{ group: any; answers: Record<string, string>; onChange: (id: string, v: string) => void }> = ({ group, answers, onChange }) => (
    <div className="mb-8">
        {/* Heading list */}
        <div className="mb-5 p-4 bg-blue-50 rounded-2xl border border-blue-100">
            <p className="text-xs font-black text-blue-700 uppercase tracking-widest mb-3">List of Headings</p>
            <div className="space-y-1">
                {group.headings.map((h: any) => (
                    <div key={h.number} className="flex gap-2 text-xs text-slate-700">
                        <span className="font-black text-blue-700 flex-shrink-0">({h.number})</span>
                        <span>{h.text}</span>
                    </div>
                ))}
            </div>
        ...
        {group.questions.map((q: any) => (
            <div key={q.id} className="mb-4 p-4 bg-white rounded-2xl border border-slate-200 shadow-sm">
                <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Question {q.number}</p>
                <p className="text-sm font-medium text-slate-800 mb-3">Section {q.section}</p>
                <div className="flex flex-wrap gap-2">
                    {group.headings.map((h: any) => (
                        <button key={h.number} onClick={() => onChange(q.id, h.number)}
                            className={`px-3 py-1.5 rounded-xl text-xs font-black border transition-all ${answers[q.id] === h.number ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-slate-50 text-slate-500 border-slate-200 hover:border-blue-300 hover:text-blue-600'}`}>
                            {h.number}
                        </button>
                    ))}
                </div>
...
const MatchingFeaturesGroup: React.FC<{ group: any; answers: Record<string, string>; onChange: (id: string, v: string) => void }> = ({ group, answers, onChange }) => (
    <div className="mb-8">
        {/* Feature list */}
        <div className="mb-5 p-4 bg-amber-50 rounded-2xl border border-amber-100">
            <p className="text-xs font-black text-amber-700 uppercase tracking-widest mb-3">Features / Categories</p>
            <div className="space-y-1">
                {group.features.map((f: any) => (
                    <div key={f.label} className="flex gap-2 text-xs text-slate-700">
                        <span className="font-black text-amber-700 flex-shrink-0">{f.label}</span>
                        <span>{f.text}</span>
                    </div>
                ))}
            </div>
        ...
        {group.questions.map((q: any) => (
            <div key={q.id} className="mb-4 p-4 bg-white rounded-2xl border border-slate-200 shadow-sm">
                <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Question {q.number}</p>
                <p className="text-sm font-medium text-slate-800 mb-3">{q.description}</p>
                <div className="flex flex-wrap gap-2">
                    {group.features.map((f: any) => (
                        <button key={f.label} onClick={() => onChange(q.id, f.label)}
                            className={`px-4 py-2 rounded-xl text-xs font-black border transition-all ${answers[q.id] === f.label ? 'bg-amber-500 text-white border-amber-500 shadow-md' : 'bg-slate-50 text-slate-500 border-slate-200 hover:border-amber-300 hover:text-amber-600'}`}>
                            {f.label}
                        </button>
                    ))}
                </div>
...
// ─── Notes Completion Renderer ───────────────────────────────────────────────
const NotesCompletionGroup: React.FC<{ group: any; answers: Record<string, string>; onChange: (id: string, v: string) => void }> = ({ group, answers, onChange }) => (
    <div className="mb-8">
        <div className="border border-slate-300 rounded-2xl overflow-hidden">
            {group.boxTitle && (
                <div className="bg-slate-100 border-b border-slate-300 px-5 py-3 text-center">
                    <p className="text-sm font-black text-slate-800">{group.boxTitle}</p>
                </div>
            )}
            <div className="p-5 space-y-4 bg-white">
                {group.notes.map((note: any, i: number) => (
                    <div key={i} className="flex items-center flex-wrap gap-1 text-sm text-slate-800 leading-relaxed">
                        <span className="mr-1">•</span>
                        {note.text && <span>{note.text}</span>}
                        {note.inline && (
                            <span className="inline-flex items-center gap-1">
                                <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-blue-600 text-white text-[9px] font-black">{note.inline.number}</span>
                                <input
                                    type="text"
                                    value={answers[note.inline.inputId] || ''}
                                    onChange={e => onChange(note.inline.inputId, e.target.value)}
                                    className="w-32 px-2 py-1 border border-slate-300 rounded-lg text-xs font-medium focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                                />
                            </span>
                        )}
...
const TableCompletionGroup: React.FC<{ group: any; answers: Record<string, string>; onChange: (id: string, v: string) => void }> = ({ group, answers, onChange }) => (
    <div className="mb-8">
        {group.tableTitle && (
            <h4 className="text-sm font-black text-slate-800 text-center mb-3">{group.tableTitle}</h4>
        )}
                ...
                <thead>
                    <tr>
                        {group.headers.map((h: string, i: number) => (
                            <th key={i} className="border border-slate-300 bg-slate-100 px-3 py-2 text-left font-black text-slate-700">{h}</th>
                        ))}
                    </tr>
                </thead>
                <tbody>
                    {group.rows.map((row: any, ri: number) => (
                        <tr key={ri}>
                            <td className="border border-slate-300 px-3 py-3 font-bold text-slate-700 bg-slate-50 align-top w-28">{row.rowHeader}</td>
                            {row.cells.map((cell: any, ci: number) => (
                                <td key={ci} className="border border-slate-300 px-3 py-3 align-top">
                                    {cell.bullets.map((bullet: any, bi: number) => (
                                        <div key={bi} className="flex items-center flex-wrap gap-1 mb-1 last:mb-0 text-slate-700">
                                            <span className="mr-0.5">•</span>
                                            {bullet.text && <span>{bullet.text}</span>}
                                            {bullet.inline && (
                                                <span className="inline-flex items-center gap-1">
                                                    <span className="inline-flex items-center justify-center w-4 h-4 rounded-full bg-blue-600 text-white text-[8px] font-black">{bullet.inline.number}</span>
                                                    <input
                                                        type="text"
                                                        value={answers[bullet.inline.inputId] || ''}
                                                        onChange={e => onChange(bullet.inline.inputId, e.target.value)}
                                                        className="w-24 px-2 py-0.5 border border-slate-300 rounded-lg text-xs font-medium focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                                                    />
                                                </span>
                                            )}
        ...
        {passage.questionGroups.map((group: any, idx: number) => (
            <div key={idx} className="mb-2">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 mb-4">
                    <p className="text-xs text-slate-600 font-medium whitespace-pre-line leading-relaxed">{group.instruction}</p>
                </div>
                {group.type === 'true_false_ng' && (
                    <TFNGGroup group={group} answers={answers} onChange={onChange}
                        labels={group.instruction?.includes('YES') ? ['YES', 'NO', 'NOT GIVEN'] : ['TRUE', 'FALSE', 'NOT GIVEN']} />
                )}
                {group.type === 'multiple_choice' && <MCQGroup group={group} answers={answers} onChange={onChange} />}
                {group.type === 'short_answer' && <ShortAnswerGroup group={group} answers={answers} onChange={onChange} />}
                {group.type === 'matching_headings' && <MatchingHeadingsGroup group={group} answers={answers} onChange={onChange} />}
                {group.type === 'matching_features' && <MatchingFeaturesGroup group={group} answers={answers} onChange={onChange} />}
                {group.type === 'notes_completion' && <NotesCompletionGroup group={group} answers={answers} onChange={onChange} />}
                {group.type === 'table_completion' && <TableCompletionGroup group={group} answers={answers} onChange={onChange} />}
            </div>
        ))}
    </div>
);
...
const ReadingTestEngine: React.FC = () => {
        ...
        supabase.from('test_answer_keys').select('answers').eq('test_id', TEST_ID).maybeSingle()
            .then(({ data }) => {
                if (data?.answers && Object.keys(data.answers).length > 0) {
                    ...
                    Object.entries(data.answers).forEach(([k, v]) => { key[Number(k)] = String(v); });
                    setAnswerKey(key);
                }
            });
    }, []);
        ...
        testData.passages.forEach(p =>
            p.questionGroups.forEach((g: any) =>
                getGroupQuestions(g).forEach(q => {
                    ...
                    if (ans && ans.trim() !== '') {
                        attempted++;
                        const key = answerKey[q.number];
                        if (key && key.trim() !== '') {
                            const validKeys = key.split('|').map(k => k.trim().toLowerCase());
                            if (validKeys.includes(ans.trim().toLowerCase())) correct++;
                        }
                    }
                })
            )
        );
        const getBand = (n: number) => n >= 39 ? '9.0' : n >= 37 ? '8.5' : n >= 35 ? '8.0' : n >= 33 ? '7.5' : n >= 30 ? '7.0' : n >= 27 ? '6.5' : n >= 23 ? '6.0' : n >= 20 ? '5.5' : n >= 16 ? '5.0' : '< 5.0';
        const bandScore = Object.keys(answerKey).length > 0 ? getBand(correct) : null;
    ...
    return (
        <div
            className="h-screen overflow-hidden bg-white font-sans text-slate-900 flex flex-col"
        ...
        >
                ...
                <div className="fixed inset-0 z-[400] flex items-center justify-center" style={{ background: 'rgba(15,23,42,0.85)', backdropFilter: 'blur(6px)' }}>
                    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
                        <div className="bg-[#f1d4d4] px-8 py-6 text-center">
                            ...
                            <h2 className="text-xl font-black text-slate-800">IELTS Academic Reading</h2>
                            <p className="text-xs font-medium text-slate-600 mt-1">60 minutes · 3 passages · 40 questions</p>
                        </div>
                        <div className="px-8 py-6">
                            <ul className="space-y-3 mb-6">
                                {['📝 Read passages carefully', '⏱ 60 minutes total time', '💡 You can navigate between passages'].map((item, i) => (
                                    <li key={i} className="flex items-center gap-3 text-sm font-medium text-slate-700"><span>{item}</span></li>
                                ))}
                            </ul>
                            <button
                                onClick={() => { setShowFullscreenPrompt(false); document.documentElement.requestFullscreen?.().catch(() => { }); }}
                                className="w-full py-3 bg-slate-800 hover:bg-slate-900 text-white font-black text-sm rounded-xl transition-colors"
                            >
                                Start Reading Test
                            </button>
                            <button onClick={() => setShowFullscreenPrompt(false)} className="w-full mt-2 py-2 text-xs font-bold text-slate-400 hover:text-slate-600 transition-colors">
                                Skip — start without fullscreen
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {showFinishModal && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-8 text-center">
                        ...
                        <p className="text-sm text-slate-500 font-medium mb-6">Once submitted, you cannot change your answers.</p>
                        <div className="flex gap-3">
                            <button onClick={() => setShowFinishModal(false)} className="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-bold rounded-xl transition-colors">Cancel</button>
                            <button onClick={finishTest} className="flex-1 py-3 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-xl transition-colors">Yes, Finish</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            <header className="fixed top-0 left-0 right-0 h-14 bg-[#f1d4d4] flex items-center justify-between px-6 z-50 border-b border-rose-200">
                <div className="flex flex-col justify-center h-full">
                    <span className="text-[11px] font-black uppercase tracking-wider text-slate-800 leading-none">{testData.title}</span>
                    <div className="flex items-center gap-3 mt-0.5">
                        <span className={`text-[11px] font-bold tabular-nums ${timeLeft < 600 ? 'text-rose-600 animate-pulse' : 'text-slate-700'}`}>
            ...
            {/* ── Split panel: Passage left | Questions right ── */}
            <main className="flex flex-1 mt-14 mb-16" style={{ height: 'calc(100vh - 120px)' }}>
                {/* Passage */}
                <div className="w-1/2 border-r border-slate-200 overflow-y-auto p-8 bg-white" style={{ height: '100%' }}>
                    <div className="max-w-xl mx-auto">
                        <div className="flex items-center gap-2 mb-4">
                            <span className="text-[10px] font-black text-blue-600 bg-blue-50 border border-blue-100 px-2 py-1 rounded-lg uppercase tracking-wider">{passage.name}</span>
                            <span className="text-[10px] text-slate-400 font-bold">Q{passage.questionRange[0]}–{passage.questionRange[1]}</span>
                        </div>
                ...
                {/* Questions */}
                <div className="w-1/2 overflow-y-auto p-6 bg-slate-50" style={{ height: '100%' }}>
                    <div className="max-w-xl mx-auto">
                        <h3 className="text-sm font-black text-slate-900 uppercase tracking-wider mb-6 flex items-center gap-2">
                            <span className="w-5 h-5 rounded-full bg-blue-600 text-white text-[9px] flex items-center justify-center font-black">{activePassage + 1}</span>
                            Part {activePassage + 1} — Questions {passage.questionRange[0]}–{passage.questionRange[1]}
                        </h3>
                        <QuestionPanel passage={passage} answers={answers} onChange={handleAnswer} />
            ...
            <footer className="fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-slate-300 flex items-center px-4 z-50">
                <div className="flex gap-8 lg:gap-16 w-full px-4 justify-center">
                    {testData.passages.map((passage: any, index: number) => {
                        const answered = countAnsweredInPassage(passage);
                        const total = totalQInPassage(passage);
                        const isActive = activePassage === index;
                        // Build flat list of questions in this passage
                        const questionList: { id: string; number: number }[] = [];
                        passage.questionGroups.forEach((g: any) => {
                            questionList.push(...getGroupQuestions(g));
                        });
...
export default ReadingTestEngine;
