...
const Dashboard: React.FC = () => {
  ...
  // Days & progress — fetched from Supabase
  const [unlockedDays, setUnlockedDays] = useState<number[]>([]);
  const [completionData, setCompletionData] = useState<Record<number, Progress>>({});
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      ...
      try {
        // ── Unlocked Days ─────────────────────────────────────────────
        const courseKey = isIELTS ? 'ielts' : 'hicu';
        const { data: dayRows } = await supabase
          .from('batch_unlocked_days')
          .select('day_number')
          .eq('batch_id', userBatchId)
          .eq('course_type', courseKey);

        if (dayRows && dayRows.length > 0) {
          setUnlockedDays(dayRows.map((r: any) => r.day_number));
        }

        // ── Homework Progress ──────────────────────────────────────────
        if (currentUserId && currentUserId !== 's_demo') {
          const { data: progRows } = await supabase
            .from('homework_progress')
            .select('day_number, speaking, listening, reading, writing')
          ...
          if (progRows && progRows.length > 0) {
            ...
            progRows.forEach((r: any) => {
              map[r.day_number] = { s: r.speaking, l: r.listening, r: r.reading, w: r.writing };
            });
    ...
    fetchProgress();
  }, [isIELTS, userBatchId, currentUserId]);
          ...
          {days.map((day) => {
            ...
            return (
              <button
                ...
                className={`group relative flex flex-col items-center justify-between gap-6 pt-8 pb-5 px-5 rounded-[2rem] transition-all duration-500 border-2 ${unlocked
              ...
              >
                {unlocked && (
                  <div className={`absolute -top-3 -right-3 p-2 rounded-full shadow-xl border-4 border-slate-50 animate-in zoom-in duration-500 ${completed ? 'bg-emerald-500 text-white' : isClassMode ? 'bg-slate-100 text-slate-400' : 'bg-amber-500 text-white'
                    }`}>
                    {completed ? (
                ...
                <div className="flex flex-col items-center gap-2">
                  <span className={`text-[10px] font-black uppercase tracking-[0.25em] transition-colors ${isClassMode ? 'text-indigo-400 group-hover:text-indigo-600' : 'text-slate-400 group-hover:text-blue-500'
                    }`}>
                    {labelType}
                  </span>
                  ...
                  {isClassMode ? (
                    ...
                    <div className="flex justify-between gap-1">
                      {[
                        ...
                        <div
                          key={task.label}
                          className={`h-6 w-6 rounded-lg flex items-center justify-center text-[10px] font-black transition-all ${task.done
